...
AS $$
	...
	INNER JOIN training.dim_cards cards ON tr.card_num = cards.card_num
	INNER JOIN training.dim_accounts da ON cards.account_num = da.account_num
	INNER JOIN training.dim_clients dc  ON da.client = dc.client_id
	...
	(	
		...
		(	
		   SELECT tr2.card_num,
			...
			INNER JOIN training.dim_terminals dt1 ON tr2.terminal = dt1.terminal_id 
		...
		INNER JOIN training.dim_terminals dt2 ON t2.terminal = dt2.terminal_id 
		WHERE time_delta <= interval '1 hour' AND prev_city <> cur_city
	) t3
	INNER JOIN training.dim_cards dc    ON t3.card_num = dc.card_num 
	INNER JOIN training.dim_accounts da ON dc.account_num = da.account_num 
	INNER JOIN training.dim_clients dc2 ON da.client = dc2.client_id;
	...
	INNER JOIN training.dim_cards dc    ON q4.card_num = dc.card_num 
	INNER JOIN training.dim_accounts da ON dc.account_num = da.account_num 
	INNER JOIN training.dim_clients dc2 ON da.client = dc2.client_id;
